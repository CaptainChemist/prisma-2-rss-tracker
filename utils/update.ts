import { BUNDLES_QUERY, BUNDLE_QUERY, FEEDS_QUERY, FEED_QUERY, SAVED_ARTICLES_QUERY, SAVED_ARTICLE_QUERY } from './api/graphql/queries';
...
export const updateCache = (isFeed, action) => (store, { data }) => {
  ...
  try {
    ...
    store.writeQuery({
      ...
      data: { [isFeed ? 'feeds' : 'bundles']: [...currentItems.filter(o => o.id !== item.id), item] },
    });
  } catch (e) {}
};
...
export const updateSavedArticleCache = action => (store, { data }) => {
  ...
  try {
    ...
    store.writeQuery({
      ...
      data: { savedArticles: action === 'delete' ? savedArticles.filter(o => o.id !== item.id) : [...savedArticles, item] },
    });
  } catch (e) {}
};
