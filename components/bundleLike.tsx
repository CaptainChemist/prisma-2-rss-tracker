...
export const BundleLike = ({ bundle }) => {
  const [likeBundleMutation, { loading: likeBundleLoading }] = useMutation(
    LIKE_BUNDLE_MUTATION
  );
  const { user, loading } = useFetchUser();

  if (likeBundleLoading || loading) {
    return (
      <svg
  ...
  const likeMatches = bundle.likes.filter(
    (oneLike) => oneLike.auth0 === user.sub
  );
  ...
  return (
    ...
    >
      <svg
        ...
        viewBox='0 0 24 24'
        stroke='currentColor'
        className={`h-6 w-6 ${hasMatch ? `text-gray-500` : `text-red-500`}`}
      >
        <path
          strokeLinecap='round'
